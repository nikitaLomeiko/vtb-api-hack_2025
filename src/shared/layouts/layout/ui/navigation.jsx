...
const Navigation = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-(--bg-primary) border-t border-(--border-primary) md:hidden z-90 shadow-lg">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <div className="absolute top-0 w-1 h-1 bg-(--accent-primary) rounded-full"></div>
              )}
            </button>
          )
        })}
      ...
      <div className="h-4 bg-(--bg-primary)"></div>
    </nav>
  )
}
