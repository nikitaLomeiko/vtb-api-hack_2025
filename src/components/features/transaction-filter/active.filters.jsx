        ...
        <div className="flex flex-col pb-5 sm:flex-row justify-between items-start sm:items-center gap-4 pt-4 border-t border-gray-200">
            <div className="flex flex-wrap items-center gap-2">
                ...
                {transaction.activeFilter !== 'all' && (
                    <FilterChip
                        label={transaction.activeFilter === 'credit' ? '📈 Доходы' : '📉 Расходы'}
                ...
                {transaction.dateRange.start && (
                    <FilterChip
                        label={`📅 С ${new Date(transaction.dateRange.start).toLocaleDateString('ru-RU')}`}
                ...
                {transaction.dateRange.end && (
                    <FilterChip
                        label={`📅 По ${new Date(transaction.dateRange.end).toLocaleDateString('ru-RU')}`}
