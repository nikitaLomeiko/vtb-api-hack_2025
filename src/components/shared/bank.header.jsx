...
import { getBankStyle, getBankIcon, formatBalance, getBankTotalBalance, declineAccount } from '@lib/utils/bank.utils';
        ...
        <div className="bg-white rounded-xl p-4 md:p-5">
          <div className="flex flex-col space-y-3 md:flex-row md:items-center md:justify-between md:space-y-0">
            <div className="flex items-center space-x-3 md:space-x-4 flex-1">
              <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center text-xl md:text-2xl bg-gradient-to-r ${style.gradient} text-white shadow-lg flex-shrink-0`}>
                {getBankIcon(bankItem.name)}
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-gray-900 text-lg md:text-xl truncate">{bankItem.name}</h3>
                <div className="flex items-center space-x-2 mt-1 flex-wrap gap-y-1">
                  <div className={`px-2 py-1 rounded-full ${style.bg} ${style.text} text-xs font-medium`}>
            ...
            <div className="flex items-center justify-between md:justify-end space-x-3 md:space-x-4 mt-2 md:mt-0">
              ...
              <div className="text-right md:mr-4">
                <p className={`text-lg md:text-2xl font-bold whitespace-nowrap transition-all duration-300 text-gray-600 filter-none`}>
                  {formatBalance(totalBalance)} ₽
                </p>
