    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex flex-col items-center justify-center p-6 relative overflow-hidden">
            ...
            <div className="relative z-10 bg-black/30 backdrop-blur-lg rounded-3xl p-8 md:p-12 border border-white/20 shadow-2xl">
                ...
                {/* Выбор ставки */}
                <div className="mb-8">
                    <p className="text-white text-center mb-3 text-lg">Выберите ставку:</p>
                ...
                <div className="flex gap-3 md:gap-6 mb-8 relative">
                    {reels.map((reel, index) => (
                        <div 
                            ...
                            className="relative"
                        >
                            <div className={`w-24 h-24 md:w-32 md:h-32 bg-gradient-to-br from-gray-800 to-gray-900 border-4 border-yellow-500 rounded-2xl flex items-center justify-center text-5xl md:text-6xl shadow-2xl overflow-hidden relative ${
                ...
                {/* Результат */}
                {result && (
                    <div className={`mb-4 px-6 py-4 rounded-2xl font-bold text-xl md:text-2xl text-center transition-all duration-300 border-2 ${
                ...
                <div className="bg-black/50 rounded-2xl p-4 border border-white/20">
                    <p className="text-white text-center mb-3 font-bold">Таблица выплат:</p>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                        {symbols.map((symbol, index) => (
                            <div key={symbol} className="flex items-center gap-2 bg-white/10 rounded-lg p-2">
                                <span className="text-2xl">{symbol}</span>
                                <span className="text-yellow-400 font-bold">x{getWinMultiplier(index)}</span>
                            </div>
                        ))}
                    </div>
                </div>
